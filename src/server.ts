// src/server.ts
//
// Sigma Control MCP — a standalone stdio MCP bridge. An external MCP client
// (Claude Code, a Hermes agent, OpenClaw, …) spawns this process as its MCP
// server; it connects BACK to a running SigmaLink's Control MCP socket and
// relays the MCP protocol (initialize / tools/list / tools/call).
//
//   External MCP client ──stdio──> THIS BRIDGE ──unix socket / pipe──> SigmaLink
//    (Claude / Hermes / OpenClaw)    (relays JSON-RPC)                  (ControlMcpHost;
//                                                                        forces origin:'external')
//
// Zero runtime dependencies — only Node built-ins. The bridge is a THIN relay:
// it fetches tools/list LIVE from the running app, so it never needs updating
// when SigmaLink adds or changes tools.
//
// Env vars (set by whoever spawns this, e.g. via `claude mcp add ... -e ...`):
//   SIGMA_CONTROL_SOCKET  — unix socket path (macOS/Linux) / named pipe (Windows)
//   SIGMA_CONTROL_TOKEN   — bearer token for the control.hello handshake
//   SIGMA_CONTROL_LABEL   — (optional) human label sent in the handshake; default 'external'
//
// Get the socket path + token from SigmaLink → Settings → External Control
// (the copyable connect command).
...
/** Wrap a host tools.invoke result `{ok, result, error?}` into an MCP tool-call response. */
...
/** Handle one JSON-RPC line from the MCP client (Claude / Hermes / OpenClaw). */
...
// ---------------------------------------------------------------------------
// Entry point — connect, then relay stdin ⇄ control socket.
// ---------------------------------------------------------------------------
...
export async function main(): Promise<void> {
  ...
  try {
    ...
    // Emit a well-formed MCP error before exiting so the spawner sees it.
    writeStdout(
      JSON.stringify({
        jsonrpc: '2.0',
        id: null,
